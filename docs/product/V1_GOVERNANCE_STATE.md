...
- **HEAD (state recorded against):** `ebee98d` — "feat(bdl): scoped status-only game finalizer + canonical post/pre mapping (V1-OP-5D, closes GAP-33)". This bookkeeping refresh ships as the **next** commit, so immediately after it lands this line is one commit stale by design.
...
1. ← **CURRENT** — **GAP-36 registered + V1-OP-8a authority committed** (this commit).
2. Implement **V1-OP-8a** — the bounded one-game retrieval/persistence caller. **0 paid requests.**
3. **Audit + commit** the bounded driver.
