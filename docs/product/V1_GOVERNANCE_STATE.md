...
**V1-OP-5D — BUILT + VERIFIED, but BLOCKED by GAP-33 (2026-08-01); deliverables remain UNCOMMITTED.** The scoped status-only finalizer is implemented and verified (owner `src/bdl/gameFinalizer.ts`, operator `scripts/v1_op_5d_finalize.ts`, `tests/bdl/gameFinalizer.test.ts` 16/16; typecheck clean; full suite 619 pass / 0 fail / 143 env-skip). It CANNOT run in production: a live write-free dry-run proved BDL WNBA `/games` reports completed games as `status:"post"`, which the canonical `mapBdlGameStatus` does not recognize → the owner correctly quarantines all 36 rather than guessing (GAP-33). The finalizer is NOT defective; the canonical mapper is incomplete. The dry-run left the DB byte-identical (both start-time fields + `status` + `updated_at` unchanged; zero writes); no `--apply` was performed.

**Relight execution order — CORRECTED (2026-08-01, supersedes the GAP-31-repair ordering, per the GAP-33 ruling):** (1) fix the canonical BDL status mapper — add exactly `post → final` / `pre → scheduled`, strict quarantine for every other unknown token, deterministic tests (GAP-33); (2) revalidate **V1-OP-5D** end-to-end and commit its deliverables; (3) **separate founder decision** on a one-game bounded `--apply`; (4) **separate authorization** for the 36-game run; (5) backfill those games' box scores; (6) run **V1-OP-8** validation (Gate 2 already widened to BOTH start-time fields) on a representative box-score-complete slate; (7) continue **Path C** historical repair after independent review; (8) **V1-OP-5c** before durable restoration is declared. The two duplicate script-level mappers (`v1_4b`/`v1_4e` `bdlStatusToInternal`) are NOT blockers and are deferred to V1-OP-5c's one-owner consolidation; `scripts/v1_4e_step2_forward_games.ts` remains PROHIBITED under GAP-31.
