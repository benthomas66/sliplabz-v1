...
**Status:** OWNER-APPROVED v1.0
**Method version:** `evidence_method_v1`
...
| DR-14 | Margin normalization constants per market | **PROVISIONALLY APPROVED:** `M_points = 6.0`, `M_rebounds = 3.0`, `M_assists = 2.0`, `M_threes = 1.5` | [OWNER APPROVED PROVISIONALLY — 2026-07-14] | See §I.1 "Required pre-engine validation of DR-14" — a zero-provider-credit offline sanity check against the seeded historical data is required BEFORE V1-A1-3 begins. The validation confirms the constants; it does NOT authorize the implementation agent to change them. Any proposed change returns through owner/governor review under the DR-24 version policy. |
...
| DR-27 | Abnormal-dispersion cap threshold (§I.3) | **FORMAL §11.5 DEFERRAL — no K assigned in `evidence_method_v1`.** A1 §11.5 authorizes an abnormal-dispersion penalty; the trigger threshold `K` is deliberately NOT set by intuition. Activation awaits the extended DR-14 offline distribution validation (§I.1) — after that evidence is reviewed, DR-27 RETURNS for owner/governor approval with a recommended `K` and documented expected product impact. Activating the rule later requires a method-version bump per DR-24 AND regression fixtures per A1 §12. | [OWNER APPROVED — DEFERRED PENDING OFFLINE CALIBRATION — 2026-07-14] | (1) FORMAL §11.5 DEFERRAL — never a silent omission. (2) `ABNORMAL_DISPERSION` is a RESERVED reason code with NO ACTIVE TRIGGER in `evidence_method_v1` (see §E.1 vocabulary table). (3) NO profile is capped on abnormal dispersion in `evidence_method_v1`. (4) The engine MUST NOT invent, approximate, or derive an abnormal-dispersion threshold; any engine-side attempt to do so is a halt condition. |
...
2. Contradictions: (none crossing thresholds).
...
### I.1 Required pre-engine offline validation (DR-14 constants + DR-27 calibration evidence)

Per DR-14 (provisionally approved) AND per DR-27 (formally deferred pending calibration evidence), a zero-provider-credit offline sanity check MUST run against the seeded historical season data BEFORE V1-A1-3 (engine) begins. The check reports, per launch market:

**Margin-normalizer distribution (DR-14 validation):**

- count of eligible historical margins;
- median absolute margin;
- 75th / 90th / 95th percentile absolute margin;
- proportion of observations clamping at `−1` or `+1` under the proposed normalizer;
- whether any market (particularly `player_threes`, given its 1.5 margin normalizer) receives disproportionate influence from ordinary rather than exceptional margins.

**L10 margin-dispersion distribution (DR-27 calibration evidence):**

- distribution of L10 margin standard deviation;
- median L10 margin standard deviation;
- 75th percentile of L10 margin standard deviation;
- 90th percentile of L10 margin standard deviation;
- 95th percentile of L10 margin standard deviation;
- proportion of profiles that WOULD be capped at each candidate value `K = 1.5, 2.0, 2.5, 3.0` under the trigger `stddev(L10 margins) > K × margin_normalizer(market)`;
- specific impact on `player_threes`, given its 1.5 margin normalizer;
- at least five concrete profile examples near EACH candidate cutoff (`K = 1.5, 2.0, 2.5, 3.0`), showing player, market, L10 margins, computed stddev, and would-be classification with and without the abnormal-dispersion cap.

**Governance clauses (apply to BOTH sets of validation outputs):**

- The check VALIDATES the approved DR-14 constants AND assembles the evidence base for DR-27's later decision. It DOES NOT authorize the implementation agent to change any constant or set `K`.
- Any proposed change to `M_points`, `M_rebounds`, `M_assists`, or `M_threes` returns through owner/governor review under the DR-24 version policy — a change bumps `evidence_method_v1` → `evidence_method_v2` and triggers regression fixtures per A1 §12.
- After the DR-27 calibration evidence is reviewed by the product owner, **DR-27 RETURNS for owner/governor approval with a recommended `K` and documented expected product impact.** The implementation agent MUST NOT choose `K`; that decision is the owner's.
...
**RME-4 (abnormal dispersion signal) — DEFERRED.** Amendment §11.5 requires that "explicit penalties or caps must apply to … abnormal dispersion" but does NOT require abnormal dispersion to be exposed as a dedicated read-model field. Governor cite: **A1 §11.5** authorizes the penalty; the amendment does NOT list a mandatory read-model field for it. RME-4 is DEFERRED to a later engine-side derivation and, when reached, may propose a dedicated read-model field if owner review authorizes it. See DR-27 in the Decision Register — the abnormal-dispersion threshold is FORMALLY DEFERRED pending the extended DR-14 offline distribution validation (§I.1); no `K` is assigned in `evidence_method_v1`.
...
**DR-27 — FORMALLY DEFERRED [OWNER APPROVED — DEFERRED PENDING OFFLINE CALIBRATION — 2026-07-14].** Amendment §11.5 authorizes an abnormal-dispersion penalty; the trigger threshold `K` is NOT chosen by intuition in `evidence_method_v1`. DR-27 is deliberately deferred pending the extended DR-14 offline distribution validation (§I.1) that assembles L10-margin-dispersion evidence per market. The four load-bearing consequences for `evidence_method_v1` are:

1. **FORMAL §11.5 DEFERRAL — never a silent omission.** The amendment §11.5 abnormal-dispersion requirement is ACKNOWLEDGED. It is postponed to a future method version pending owner calibration input; it is not being dropped.
...
4. **The engine MUST NOT invent, approximate, or derive an abnormal-dispersion threshold.** Any engine-side attempt to compute a threshold, to auto-tune `K`, or to emit `ABNORMAL_DISPERSION` from an inferred rule is a **halt condition**. Activation later requires (a) owner review of the §I.1 calibration evidence, (b) owner/governor approval of a recommended `K` with documented expected product impact, (c) a DR-24 method-version bump (`evidence_method_v1` → `evidence_method_v2`), AND (d) regression fixtures per A1 §12.

No other decisions remain open or deferred. Every remaining tunable has an approved value in the Decision Register above.
...
- ✅ **Normalization** stated (§B.3 margin normalizer per DR-14; pre-engine validation in §I.1).
...
- ✅ **Limitations** — the Decision Register is the limitation register; §I.3 records DR-27 as FORMALLY DEFERRED pending the extended §I.1 offline calibration validation (no `K` assigned in `evidence_method_v1`; `ABNORMAL_DISPERSION` reserved but not emitted). Zero decisions remain open.
...
*End of OWNER-APPROVED v1.0. This method authority is the single source of truth for every subsequent evidence ticket (V1-A1-2 schema, V1-A1-3 engine, V1-A1-4 templates). All decisions are resolved: DR-27 (abnormal-dispersion threshold) is FORMALLY DEFERRED per §I.3, pending the extended DR-14 offline calibration validation (§I.1); `ABNORMAL_DISPERSION` is a RESERVED, non-emitted reason code in `evidence_method_v1`. The document is final at `evidence_method_v1` awaiting governor commit authorization.*
